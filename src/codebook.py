...
class Codebook(nn.Module):
    def __init__(self, config):
        ...
        self.embedding = nn.Embedding(num_embeddings=self.num_vectors, embedding_dim=self.latent_dim) # num_vectors, latent_dim
    ...
    def forward(self, z):
        ...
        # sample the codebook
        z_q = self.embedding(min_emb_indices).view(z.shape) # bhw, latent_dim -> b, h, v, latent_dim

        # first term pushes the encoder to produce embeddings similar to code book
        # second term pushes the codebook to be similar to encoder's embedings
