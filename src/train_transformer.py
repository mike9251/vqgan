...
class Trainer:
    def __init__(self, config: DictConfig):
        ...
        if config.ddp:
            self.vqgan_transformer = DDP(
                self.vqgan_transformer, device_ids=[self.device_id], find_unused_parameters=True
            )
    ...
    def _set_optimizer(self):
        ...
        self.opt = torch.optim.AdamW(
            optim_groups, lr=self.config.lr, betas=(self.config.beta1, self.config.beta2)
        )
    ...
    def _save_checkpoint(self, epoch):
        ...
        torch.save(
            {
                "transformer": self._unwrap().transformer.state_dict(),
            ...
            with tqdm(range(steps_per_epoch), disable=not self.rank == 0) as pbar:
                for i, batch in enumerate(self.train_dataloader):
                    ...
                    if self.log_grads:
                        for n, p in self._unwrap().transformer.named_parameters():
                            if p.grad is None:
                                ...
                                m = self.running_meters.get(
                                    f"grad/{n}",
                                    RunningMeter(window_size=self.log_every, ddp=self.ddp),
                                )
                    ...
                    if i % (self.log_every + 1) == 0:
                        with torch.no_grad():
                            img, rec, halh_sample, full_sample = self._unwrap().log_imgs(
                                batch[0:1, ...]
                            )
...
@hydra.main(
    config_path="../configs/", config_name="vqgan_transformer_celeba.yaml", version_base="1.3"
)
